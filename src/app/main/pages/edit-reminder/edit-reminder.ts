...
export class EditReminderPage {
    ...
    constructor(private _router: Router, private _route: ActivatedRoute, private reminderService: ReminderService, private utilService: UtilService) {
    ...
    async getReminderInfo(data: any) {
        await this.utilService.showLoading();
        const self = this;
        this.reminderService.getReminder(data).then(async function (o) {
            await self.utilService.closeLoading();
            if (o.success) {
                self.reminder = {
                    id: o.reminder.id,
                    title: o.reminder.title,
                    description: o.reminder.description,
                    alert: o.reminder.alert
                }

                const d = moment(o.reminder.date).toDate();
    ...
    async accept() {
        ...
        this.reminderService.updateReminder({
            ...
            await self.utilService.closeLoading();
            if (o.success) {
                self.utilService.showSuccessToast('The reminder is successfully updated. ');
